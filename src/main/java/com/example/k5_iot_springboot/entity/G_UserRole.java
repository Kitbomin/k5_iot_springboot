...
public class G_UserRole {
    ...
    @JoinColumn(
            name = "user_id", nullable = false, foreignKey = @ForeignKey(name = "fk_user_roles_user")
    )
    private G_User user;
    ...
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(
            name = "role_name", nullable = false,
            referencedColumnName = "role_name", // 대상 엔티티의 PK가 아닌 컬럼을 FK로 참조하기에 이런걸 만듬
