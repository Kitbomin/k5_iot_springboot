...
public enum C_Category {
    // 각 Enum 상수
    // : DB에 실질적으로 저장될 값(dbValue) 과 화면 표시용 값(label) 을 함께 정의할거임
    ...
    *   : String >> Enum
    *   : JSON 요청 값을 Enum으로 변환할 때, Enum 이름(name)과 DB 값 (dbValue) 를 모두 인식
    *   - 대소문자 구분 없이 값이 없거나 잘못된 경우 예외 발생됨
    */
    @JsonCreator
