...
* : JWT 인증 필터
* - 요청에서 JWT 토큰을 추출
* - request의 header에서 토큰을 추출해 검증 (유효한 경우 Security의 context 정보에 인증 정보를 설정함)
*
* cf) Spring Security 가 OncePerRequestFilter 를 상속받아 매 요청마다 실행함
* */
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            // 이미 인증된 컨텍스트가 있으면 스킵 (다른 필터가 인증처리를 한 경우임)
            ...
            // 헤더에서 토큰을 파싱하여 가져옴("Bearer " 공백 제거)
            String token =
            ...
            // 권한 문자열 - GrantedAuthority 로 매핑("ROLE_" 접두어 보장)
