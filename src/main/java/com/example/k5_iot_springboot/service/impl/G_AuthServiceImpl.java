...
//                .gender(req.gender()) -> null이 허용되니까 없어도 오류 안남
        ...
        // 기본 권한 부여
        // cf) getReferenceById: 특정 ID를 가진 엔티티의 프록시 객체를 즉시 반환해주는 메서드임
    ...
    /**
     * 로그인
     * - 인증 성공 시 Access/Refresh Token이 발급됨
...
//        cookie.setSecure(true);
    ...
    public String refreshAccessToken(String refreshToken) {
        try {
            ...
            // 3) DB에 저장된 Refresh Token 과 일치하는지 확인
            ...
            if (savedToken.isEmpty() || !savedToken.get().getToken().equals(refreshToken)) {
                throw new IllegalArgumentException("Refresh Token이 서버에 등록된 값과 일치하지 않습니다.");
            }
    ...
    public void resetPassword(MailRequest.@Valid PasswordReset req) {
        ...
        G_User user = userRepository.findByEmail(req.email())
                .orElseThrow(()-> new IllegalArgumentException("가입된 이메일이 아니에요"));
