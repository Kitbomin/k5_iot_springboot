...
public class ReservationServiceImpl implements ReservationService {
    ...
    public ResponseDto<List<ReservationResponseDto>> getReservationsByTruck(Long truckId) {
        ...
        return ResponseDto.setSuccess("해당 truckId의 예약목록을 성공적으로 응답하였습니다.", dtos);
    }

    @Override
    public ResponseDto<ReservationResponseDto> getReservation(Long truckId, Long reservationId) {
        Reservation reservation = reservationRepository.findAllByIdAndTruckId(reservationId, truckId)
                .orElseThrow(() -> new IllegalArgumentException("해당 트럭에 대한 예약을 찾을 수 없어요"));
