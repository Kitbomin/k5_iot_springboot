...
*       : 매핑한 핸들러 실행을 위한 어댑터를 찾고 Controller가 실행될 수 있는 형태로 어댑터가 변환해줌
*
...
*           - 구조를 스스로 만들고, 규칙도 스스로 정해야함
*
*       2) 기능/URI 마다 여러 서블릿을 만들거나, 직접 프론트 컨트롤러 패턴(서블릿 1개 당 내부 분기)을 구현해야함
*
*
* 2. 스프링부트(MVC)
*       1) DispatcherServlet 하나가 관문
*           - 그 아래는 컨트롤러 메서드 단위로 라우팅 됨
*           - 공통 보일러 플레이트(바인딩, 검증, 예외, 직렬화)를 프레임워크가 제공함
*
*       2) 서블릿 레벨이 DispatcherServlet 1개면 충분함 (스프링이 자동 등록해줌)
*           - 컨트롤러 클래스/메서드는 여러개가 만들어짐
*               >> 애플리케이션 코드 레벨
*                   "서블릿 여러개"가 아닌 POJO(Plain Old Java Obj) 단위라 가볍고 일관적임
*
*           cf) POJO 메서드
*               : 클래스 내부에 정의된 메서드를 의미
*               - 특정 프레임워크나 기술에 종속되지 않고 순수하게 데이터와 로직을 캡슐화하는 객체를 의미함
*
*/
