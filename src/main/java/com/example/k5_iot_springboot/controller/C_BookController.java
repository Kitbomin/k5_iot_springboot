...
//        return ResponseEntity.created(location).body(result);
    ...
    // - 검색, 필터링, 정렬, 페이지네이션 등 조건을 전달할 때 사용함
    // >> 값이 선택적일 수 있음 (required=false 설정 가능함 - 필수가 아니란거임)
    // >> 여러 개의 값을 받을 수 있음 (&로 나열해서 받을 수 있음)

    // @PathVariable
    // : URI의 경로의 일부를 변수로 인식해서 값으로 받음
    // - 고정된 리소스의 식별자를 전달할 때 사용함
    // >> 값이 필수임
    // >> 리소스를 식별하는 역할이므로 RESTful API에서 많이 사용됨
     ...
     // cf) 숫자로 변환할 수 없는 데이터 전달 시 400 Bad Request 발생
    {
        ...
        return ResponseEntity.status(books.getMessage().equals("SUCCESS") ? HttpStatus.OK : HttpStatus.BAD_REQUEST).body(books);
    }
